...
export default function RecordingCompletedPage() {
  ...
  const handleDeliveryComplete = async (id: string) => {
    const ok = confirm(
      "この案件を納品完了にしますか？\n納品日も今日の日付で自動入力されます。"
    );
    ...
    if (error) {
      alert("納品完了エラー: " + error.message);
    } else {
      fetchProjects();
    }
  };

  const handleCancelRecordingComplete = async (id: string) => {
    const ok = confirm("収録完了を取り消して、通常の案件一覧に戻しますか？");
    ...
    if (error) {
      alert("収録完了取り消しエラー: " + error.message);
    } else {
      fetchProjects();
    }
  };
  ...
  const sortedProjects = [...filteredProjects].sort((a, b) => {
    const aTime = a.recording_completed_at || "";
    const bTime = b.recording_completed_at || "";
    return bTime.localeCompare(aTime);
  });

  return (
    <div className="page">
      <h1 className="page-title">収録完了一覧</h1>
          ...
          return (
            <li key={p.id} className="card card-complete">
              ...
              >
                ...
                <button type="button" onClick={() => handleDeliveryComplete(p.id)}>
                  納品完了
                </button>
                ...
                <button
                  ...
                  onClick={() => handleCancelRecordingComplete(p.id)}
                >
                  収録完了を取り消す
                </button>
              </div>
